...
% Load data
[volumes, frequencies] = ExtractBottleVolumeAndFrequency(path, 'RandomBottle_');
...
figure;
scatter(volumes, frequencies, 'bo', 'DisplayName', 'Measured data');
hold on;
plot(volumes_fit, frequencies_fit, 'r-', 'LineWidth', 2, 'DisplayName', sprintf('Fitted polynomial (order %d)', order1));
xlabel('Volume [ml]');
ylabel('Frequency [Hz]');
...
order2 = 6; % Order of the polynomial
[p2, frequencies_fit, volumes_fit] = fit_polynomial(sortedFrequencies, sortedVolumes, order2);

% Plot the fitted curve and original data
figure;
scatter(sortedFrequencies, sortedVolumes, 'bo', 'DisplayName', 'Measured data');
hold on;
plot(frequencies_fit, volumes_fit, 'r-', 'LineWidth', 2, 'DisplayName', sprintf('Fitted polynomial (order %d)', order2));
xlabel('Frequency [Hz]');
ylabel('Volume [ml]');
